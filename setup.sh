...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Available options: --non-interactive, --force-overwrite, --append, --skip-packages"
      exit 1
      ;;
  esac
done
...
if [ -n "${CI:-}" ] || [ -n "${GITHUB_ACTIONS:-}" ]; then
  echo -e "${BLUE}CI environment detected. Adjusting settings...${NC}"
  INTERACTIVE=false
  SKIP_PACKAGES=false  # We still want to test package installation in CI
fi
...
handle_config_file() {
  ...
  # Check if we're in append mode first
...
install_packages() {
  if [ "$SKIP_PACKAGES" = true ]; then
    echo -e "${YELLOW}Skipping package installation (--skip-packages flag)${NC}"
    return 0
  fi
  ...
  if [ -f "$DOTFILES_DIR/brew_packages.txt" ]; then
    # In CI, install packages with timeout and error handling
    ...
    while IFS= read -r package; do
      ...
      if [[ -n "$package" && ! "$package" =~ ^[[:space:]]*# ]]; then
        ...
        if [ -n "${CI:-}" ]; then
          # In CI, use timeout and don't exit on failure
          if timeout 300 brew install "$package" 2>/dev/null; then
    ...
    if [ ${#failed_packages[@]} -gt 0 ]; then
      echo -e "${YELLOW}Failed to install packages: ${failed_packages[*]}${NC}"
    fi
  else
    echo -e "${RED}Error: brew_packages.txt not found in $DOTFILES_DIR${NC}"
    echo -e "${YELLOW}Falling back to essential packages...${NC}"
    brew install tmux bat || echo -e "${YELLOW}Failed to install fallback packages${NC}"
  fi
}
...
install_tpm() {
  ...
  if [ ! -d "$HOME/.tmux/plugins/tpm" ]; then
    ...
    if git clone https://github.com/tmux-plugins/tpm "$HOME/.tmux/plugins/tpm"; then
      ...
      echo -e "${YELLOW}Failed to install TPM${NC}"
    fi
  else
...
# Function to source zshrc (skip in CI)
source_zshrc() {
  if [ -n "${CI:-}" ]; then
    echo -e "${YELLOW}Skipping .zshrc sourcing in CI environment${NC}"
    return 0
  fi
...
# Source zsh configuration (skip in CI)
source_zshrc
...
if [ -n "${CI:-}" ]; then
  echo -e "${BLUE}CI setup complete. Symlinks created and packages installed.${NC}"
else
